# 📰 Daily Financial News Bot | AI 專業經理人晨報

每天早上醒來，面對海量的國內外財經新聞，我們需要的通常不是「更多的資訊」，而是「精準的洞察」。

這個專案是我為了解決日常資訊焦慮而開發的輕量級資料管線 (Data Pipeline)。透過自動化抓取多方權威財經媒體的 RSS，交由 LLM 進行雜訊過濾與情緒分析，並在每天早上開盤前，準時將帶有「潛在市場影響 (Actionable Insights)」的視覺化卡片推播到 Discord。

這不僅僅是一個推播機器人，也是一個即時金融情緒分析處理模組的基礎雛形。

## 💡 為什麼做這個專案？

- **資訊過載**：手動瀏覽 WSJ、CNN、鉅亨網等十幾個網站太耗時。
- **缺乏重點**：一般的新聞摘要只是縮短字數，但我更在乎的是「這對總體經濟或產業板塊有什麼影響 (So What?)」。
